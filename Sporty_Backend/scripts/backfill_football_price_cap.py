"""One-time backfill applying the lowered football price cap retroactively.

`app/services/pricing/repricing.py`'s football/cricket max_cost dropped from
20.0 to 17.0 (2026-07-15) after a repricing re-run bug (fixed in the same
change — see recalculate_player_prices' last_priced_points guard) let a
daily cron reapply the same unchanged delta every run with no new match
data, pinning ~27% of the football pool at the old 20.0 ceiling.

This script caps existing football players whose cost is already above the
new ceiling down to it, and writes a PlayerPriceHistory row per player so
the drop is auditable the same way organic repricing is (transfer_window_id
left null, algorithm_version="cap_backfill_2026_07" — this wasn't derived
from a window's stats, just a policy-ceiling correction).
...
Run manually (verify with --dry-run first):
    venv/bin/python scripts/backfill_football_price_cap.py --dry-run
    venv/bin/python scripts/backfill_football_price_cap.py
"""
...
def main() -> int:
    ...
    new_cap = SPORT_POLICIES["football"].max_cost

    db = SessionLocal()
    try:
        sport = db.query(Sport).filter(Sport.name == "football").first()
        if sport is None:
            print("No 'football' sport row found, nothing to backfill")
            return 0
        ...
        if not players:
            print(f"No football players above the {new_cap} cap, nothing to do.")
            return 0
