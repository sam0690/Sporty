  ...
  return (
    <section className="mt-8 space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-lg text-black">Bench ({benchPlayers.length})</h3>
        <span className="text-xs text-secondary">Start players to fill pitch spots</span>
      </div>

      {benchPlayers.length === 0 ? (
        <div className="rounded-[3px] border border-accent/20 bg-white p-4 text-sm text-secondary">
                ...
                <article key={player.id} className="w-64 rounded-[3px] border border-accent/20 bg-white p-3 md:w-auto">
                  ...
                  <div className="mt-3 flex items-center justify-end">
                    <button
                      ...
                      className="rounded-[3px] bg-primary px-3 py-1.5 text-xs text-white transition hover:bg-primary-700 disabled:cursor-not-allowed disabled:bg-accent/40 disabled:text-secondary"
                    >
                      Start
                    </button>
                  </div>
