    ...
    <header className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="text-right">
          <p className="text-sm text-text-secondary">Budget: ${budget}</p>
      ...
      <div className="space-y-2">
        <p className="text-sm text-text-secondary">Step {step} of {totalSteps}: {stepLabel(step)}</p>
        <div className="h-2 rounded-full bg-gray-200">
          <div className="h-2 rounded-full bg-primary-500" style={{ width: `${progress}%` }} />
        </div>
        <p className="text-xs text-text-secondary">{selectedCount}/{requiredCount} players selected</p>
      </div>
    </header>
  );
}
