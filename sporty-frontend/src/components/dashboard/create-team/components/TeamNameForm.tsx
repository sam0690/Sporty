...
export function TeamNameForm({ teamName, onTeamNameChange, onSubmit, onBack }: TeamNameFormProps) {
  return (
    <section className="mx-auto max-w-md space-y-4 rounded-lg bg-surface-100 p-6 shadow-card">
      <h2 className="text-xl font-semibold text-text-primary">Step 2: Name Your Team</h2>

      <div>
        <label htmlFor="team-name" className="mb-1 block text-sm font-medium text-text-primary">Team Name</label>
        <input
          id="team-name"
          value={teamName}
          maxLength={30}
        ...
        />
        <p className="mt-1 text-right text-xs text-text-secondary">{teamName.length}/30</p>
      </div>
