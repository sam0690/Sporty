...
export function PredictionCard({ prediction }: PredictionCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="flex h-3 w-full overflow-hidden rounded-full bg-[rgba(11,18,32,0.05)]">
        {segments.map((s) => (
          <div
            key={s.key}
        ...
        {segments.map((s) => (
          <div key={s.key} className="text-center">
            <div className="flex items-center justify-center gap-1.5">
              ...
              />
              <span className="font-barlow-condensed text-[11px] font-bold uppercase tracking-[1px] text-[#6B7280]">
                {s.label}
              </span>
            </div>
            <p className="mt-1 font-bebas text-2xl leading-none tracking-[1px] text-[#0B1220]">
              {pct(s.value)}%
            </p>
          </div>
        ))}
      ...
      <div className="mt-4 border-t border-[rgba(11,18,32,0.06)] pt-2.5 text-right text-[10px] uppercase tracking-wider text-[#6B7280]">
